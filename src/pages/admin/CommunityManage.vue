<template>
  <q-page id="adminItinerary">
          ...
          <q-form v-model='form.valid' @submit.prevent='submitForm'>
            <q-card flat>
              <div class="row">
                ...
                <div class="col" col="6">
                  <q-input rounded flat primary standout v-model="form.startDay" :label="$t('發文日期')" class="q-ma-md"
                    style="box-shadow:none"></q-input>
                </div>
              </div>
              <div class="row">
                <div class="col" col="10">
                <q-input rounded flat primary standout v-model="form.title" :label="$t('社區公告標題')" class="q-ma-md" style="box-shadow:none"></q-input>
              </div>
                <!-- <div class="col" col="6">
              ...
              <div class="row">
                ...
                <div class="col" col="6">
                  <div class="col" col="6">
                    <q-file v-model="form.image" rounded standout class="q-ma-md" counter :label="$t('文章圖片')"></q-file>
                  </div>
                </div>
              </div>
              <div class="row">
                <div class="col" col="12">
                  <q-input v-model="form.description" rounded standout class="q-ma-md" label="公告詳情" autogrow type="textarea" />
                </div>
              </div>
    ...
    <!-- 上下架用switch -->
    <div class="q-pa-md">
      <q-table :grid="$q.screen.lt.md" :title="$t('社區公告')" :rows="communities" :columns="columns" row-key="name"
        ...
        <template v-slot:header-selection="scope">
          <q-toggle v-model="scope.selected" :key="communities.sell" />
        </template>
          ...
          <q-dialog v-model="confirm" persistent>
            <q-card>
              ...
              <q-card-actions align="right">
                <q-btn flat color="primary" icon="mdi-check-circle" @click="deleteCommunity(edit.row._id)"
                  v-close-popup />
          ...
          <div class="q-pa-xs col-xs-12 col-sm-6 col-md-4 col-lg-3 grid-style-transition"
            ...
            <q-dialog v-model="confirmSm" persistent>
              <q-card>
                ...
                <q-card-actions align="right">
                  <q-btn flat color="primary" icon="mdi-check-circle" @click="deleteCommunity(card.row._id)"
                    v-close-popup />
                  <q-btn flat color="primary" icon="mdi-close-circle" v-close-popup />
                </q-card-actions>
              </q-card>
...
const openDialog = (_id) => {
  form._id = _id
  const idx = _id === '' ? -1 : communities.findIndex(activity => activity._id === _id)
  if (idx > -1) {
    form.name = communities[idx].name
    // form.price = activities[idx].price
    // form.category = products[idx].category
    form.sell = communities[idx].sell
    form.description = communities[idx].description
    form.title = communities[idx].title
    form.startDay = communities[idx].startDay
    form.endDay = communities[idx].endDay
  } else {
    form.name = ''
    // form.price = 0
    // form.category = ''
...
const submitForm = async () => {
  ...
  try {
    if (form._id.length === 0) {
      const { data } = await apiAuth.post('/community/', fd)
...
const deleteCommunity = async (communityId) => {
  try {
    await apiAuth.delete('/community/' + communityId)
    await Swal.fire({
      icon: 'success',
      title: '成功',
      text: '活動刪除成功'
...
const init = async () => {
  try {
    const { data } = await apiAuth.get('/community/all')
